...
class WorkflowNodeContainers extends React.Component<Props, {selectedSidecar: string}> {
    ...
    public render() {
        ...
        if (!template || (!template.container && !template.pod && !template.script)) {
            return (
                <div className='white-box'>
                    <div className='row'>
        ...
        const containers = (template.pod ? template.pod.graph || template.pod.sequence : []).concat(template.sidecars || []);
