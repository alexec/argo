				...
				for _, x := range template.Pod.GetGraph() {
					if x.Name == containerName {
						for _, y := range x.Dependencies {
							println(fmt.Sprintf("waiting for dependency %s", y))
							for {
								...
								if err != nil {
									return fmt.Errorf("failed to read exit-code of dependency %s: %w", y, err)
								}
								if exitCode != 0 {
									return fmt.Errorf("dependency %s exited with non-zero code: %d", y, exitCode)
								}
								break
							}
						}
